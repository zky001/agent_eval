...
- **多轮工具代理评估（agent_loop）**：平台内置模拟工具执行器，模型进行真实的 Action → Observation 多轮循环，完整记录执行轨迹，按业界标准的「任务完成 + 工具序列/参数 + 效率」三维度评分，支持有状态工具流程（如重启后状态变化）
- **11 种评估类型**：涵盖工具调用、多步规划、ReAct 推理等 Agent 核心能力，以及 LLM 裁判评分（开放式任务）
...
- **成本追踪**：模型配置输入/输出单价后，自动计算每次运行成本，排行榜展示各模型单次成本（性价比对比）
- **人工复核**：任意题目可人工改判并自动重算总分，原始机器评分保留可审计
- **得分趋势**：同一数据集的历史运行得分折线图，跟踪模型/提示词迭代是否回归
...
- **测试覆盖**：105 项 pytest 测试覆盖评估器、LLM 客户端与 API 全链路（`make test`）
...
| `agent_loop` | 多轮工具代理 - 模拟工具环境中的多轮任务 | 任务完成(40%) + 工具序列与参数(40%) + 效率(20%) |
...
`agent_loop` 类型的题目在 `metadata.tools` 中定义模拟工具（`name` / `description` / `parameters` / `response`，可用 `responses` 规则按参数匹配返回不同结果，`when_called_after` 可表达“重启后状态变化”这类有状态流程），`metadata.max_turns` 限制轮数；`reference_answer` 用 JSON 描述 `final_answer_keywords`、`expected_tool_sequence`、`expected_tool_args` 等期望。运行详情页可查看每题的逐轮执行轨迹。
